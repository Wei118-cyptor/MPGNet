...
class CustomRCNN(GeneralizedRCNN):
    ...
    def forward(self, batched_inputs_new: List[Dict[str, torch.Tensor]]):
        """
        Add ann_type
        Ignore proposal loss when training with image labels
        """
        # batched_inputs,td = batched_inputs_new
        # Training: batched_inputs_new is (batched_inputs, td)
        # Testing: batched_inputs_new is just batched_inputs
        if isinstance(batched_inputs_new, tuple):
            batched_inputs, td = batched_inputs_new
        else:
        ...
        if self.roi_head_name in ['StandardROIHeads', 'CascadeROIHeads']:
            proposals, detector_losses = self.roi_heads(
                images, features, proposals, gt_instances,td = td)
        else:
            proposals, detector_losses = self.roi_heads(
                images, features, proposals, gt_instances,
                ann_type=ann_type, classifier_info=classifier_info,td = td)
