...
from .zero_shot_classifier import ZeroShotClassifier, TrainableClassifier
from torch.distributions import Categorical

__all__ = ["mpgnetFastRCNNOutputLayers"]


class mpgnetFastRCNNOutputLayers(FastRCNNOutputLayers):
    @configurable
    def __init__(
            self,
        ...
        if self.use_zeroshot_cls:
            ...
            # When USE_ZEROSHOT_CLS=False, replace with TrainableClassifier
            # This allows access to zs_weight and detection_weight for loss computation 不懂
            if cls_score is not None:
                del self.cls_score
                self.cls_score = cls_score
            del self.bbox_pred
            # self.bbox_pred = nn.Linear(input_size, 4)
            num_bbox_reg_classes = 1 if self.cls_agnostic_bbox_reg else self.num_classes#false
        ...
        self.prototype_cls_score = nn.Linear(512, self.num_classes + 1)
        nn.init.normal_(self.prototype_cls_score.weight, mean=0, std=0.01)
        nn.init.constant_(self.prototype_cls_score.bias, 0)
    ...
    def from_config(cls, cfg, input_shape):
        ...
        ret.update({
            ...
            'cls_agnostic_bbox_reg': cfg.MODEL.ROI_BOX_HEAD.CLS_AGNOSTIC_BBOX_REG,#新加的
            'l2_loss_weight': cfg.MODEL.ROI_BOX_HEAD.L2_LOSS_WEIGHT,#新加的L2损失权重
            'triplet_loss_weight': cfg.MODEL.ROI_BOX_HEAD.TRIPLET_LOSS_WEIGHT,#新加的三元组损失权重
            'triplet_loss_margin': cfg.MODEL.ROI_BOX_HEAD.TRIPLET_LOSS_MARGIN#新加的三元组损失margin
        })
        if ret['use_zeroshot_cls']:
            ret['cls_score'] = ZeroShotClassifier(cfg, input_shape)
        else:
            ret['cls_score'] = TrainableClassifier(cfg, input_shape)
        return ret
    ...
    def losses(self, predictions, proposals, \
        ...
        for idx in range(len(proposals)):
            ...
            first_image_mask.append(in_group_idx == 0)
        
        if any(first_image_mask):
            first_proposals = [p for i, p in enumerate(proposals) if first_image_mask[i]]
            first_indices = [i for i, is_first in enumerate(first_image_mask) if is_first]
            cumsum_props = [0] + [sum(num_inst_per_image[:i+1]) for i in range(len(num_inst_per_image))]
            first_scores_list = []
            first_deltas_list = []
            for idx in first_indices:
                start_idx = cumsum_props[idx]
                end_idx = cumsum_props[idx + 1]
                first_scores_list.append(scores[start_idx:end_idx])
                first_deltas_list.append(proposal_deltas[start_idx:end_idx])
            
            scores = cat(first_scores_list, dim=0) if first_scores_list else scores.new_zeros((0, scores.shape[1]))
            proposal_deltas = cat(first_deltas_list, dim=0) if first_deltas_list else proposal_deltas.new_zeros((0, proposal_deltas.shape[1]))
    ...
    def image_label_losses(self, predictions, proposals, image_labels, \
                           classifier_info=(None, None, None), ann_type='image',td=None):
        '''
        Inputs:
            scores: N x (C + 1)
            image_labels B x 1
        ...
        if self.image_label_loss == 'concept_grouping':
            ...
            for i in range(len(image_labels) // self.g_size):
                ...
                concept_id = int(td[i*self.g_size])
    ...
    def _concept_grouping_loss(self, score, label, g_proposals, idx, full_label):
        ...
        proto_cls_score = self.prototype_cls_score(prototye)  # 1 x (C + 1)
        gt_classes = torch.tensor([label], dtype=torch.long, device=proto_cls_score.device)
        if self.use_sigmoid_ce:
            loss += self.sigmoid_cross_entropy_loss(proto_cls_score, gt_classes)
        else:
            loss += self.softmax_cross_entropy_loss(proto_cls_score, gt_classes)
        text_features = self.cls_score.zs_weight.transpose(0, 1)  
        text_features = F.normalize(text_features, dim=-1) 
      
        prototype_normalized = F.normalize(prototye, dim=-1)  
      
        positive_text = text_features[label:label+1, :]  # [1, 512]
        all_indices = torch.arange(text_features.shape[0], device=text_features.device)
        negative_mask = all_indices != label
        negative_texts = text_features[negative_mask, :]  # [num_classes-1, 512]
        positive_distance = 1.0 - torch.sum(prototype_normalized * positive_text, dim=-1)  # [1]
        negative_distances = 1.0 - torch.mm(prototype_normalized, negative_texts.transpose(0, 1))  # [1, num_classes-1]
      
        triplet_losses = F.relu(positive_distance.unsqueeze(1) - negative_distances + self.triplet_loss_margin)  # [1, num_classes-1]
      
        triplet_loss = triplet_losses.mean()
        
        weighted_triplet_loss = self.triplet_loss_weight * triplet_loss
        loss += weighted_triplet_loss
        if comm.is_main_process():
            storage.put_scalar('stats_triplet_loss', triplet_loss.item())
            storage.put_scalar('stats_weighted_triplet_loss', weighted_triplet_loss.item())
