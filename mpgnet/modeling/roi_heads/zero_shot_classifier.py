...
class TrainableClassifier(nn.Module):
    """
    A classifier that uses a trainable fully connected layer when USE_ZEROSHOT_CLS=False,
    but still preserves ZEROSHOT_WEIGHT and DETECTION_WEIGHT for loss computation.
    """
    @configurable
    def __init__(
            self,
        ...
        self.weight = nn.Parameter(torch.randn(num_classes + 1, input_size))
        self.bias = nn.Parameter(torch.randn(num_classes + 1))
        nn.init.normal_(self.weight, mean=0, std=0.01)
        nn.init.constant_(self.bias, 0)

        # Also create a linear layer that maps to zs_weight_dim for compatibility with concept grouping
        self.linear = nn.Linear(input_size, zs_weight_dim)

        # Load ZEROSHOT_WEIGHT and DETECTION_WEIGHT for loss computation
        if zs_weight_path == 'rand':
            zs_weight = torch.randn((zs_weight_dim, num_classes))
            nn.init.normal_(zs_weight, std=0.01)
        else:
        ...
        if detection_weight_path == 'rand':
            detection_weight = torch.randn((zs_weight_dim, num_classes))
            nn.init.normal_(detection_weight, std=0.01)#源代码zs_weight
        else:
            detection_weight = torch.tensor(
                np.load(detection_weight_path),
                dtype=torch.float32).permute(1, 0).contiguous()  # D x C
    ...
    def forward(self, x, ann_type='box', classifier=None):
        '''
        Use trainable fully connected layer for classification
        '''
        if x.dim() > 2:
    ...
    def forward_without_linear_mapping(self, x, classifier=None):
        """
        For concept grouping: use zs_weight for feature similarity computation
        """
        if classifier is not None:
            zs_weight = classifier.permute(1, 0).contiguous()  # D x C'
            zs_weight = F.normalize(zs_weight, p=2, dim=0) \
